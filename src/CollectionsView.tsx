...
function yearRange(c: RommCollection): string {
  const start = c.created_at ? new Date(c.created_at).getFullYear() : NaN;
  const end = c.updated_at ? new Date(c.updated_at).getFullYear() : NaN;
  if (Number.isNaN(start)) return "—";
  if (Number.isNaN(end) || start === end) return String(start);
  return `${start} – ${end}`;
}
...
/** First line under the poster: game count for autogenerated, year span otherwise. */
function metaPrimaryLine(c: RommCollection): string {
  if (c.is_virtual && c.rom_count != null && c.rom_count > 0) {
    return `${c.rom_count} game${c.rom_count === 1 ? "" : "s"}`;
  }
  return yearRange(c);
}
...
function virtualTypeLabel(c: RommCollection): string {
  if (c.collection_type === "franchise") return "Franchise";
  if (c.collection_type === "collection") return "IGDB collection";
          ...
          >
            {slots.map((offset) => {
              ...
              return (
                ...
                <div className="collection-meta">
                  <span className="collection-years">{metaPrimaryLine(c)}</span>
