...
function cacheBlobKey(collectionKey: string, searchName: string, index: number): string {
...
export function getCachedSteamGridGrid(
  ...
  const blob = cacheBlobKey(collectionKey, searchName, index);
...
export function setCachedSteamGridGrid(
  ...
  pruneExpired(store.entries);
  store.entries[cacheBlobKey(collectionKey, searchName, index)] = {
    url,
    savedAt: Date.now(),
  };
